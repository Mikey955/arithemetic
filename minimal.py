"""This is an minimal function where we receive an input variable(list) to this function. The values 
...
import os
#this is the minimum function according to the menu
def minimum_function(parse_value):
    parsed = False                                                      
    while not parsed:
        try:
            ...
            minimum_value=float(parse_str[0])
            i=1
            ...
            while i < parse_len:
                ...
                if minimum_value > convert_value:
                    #if it is we assign it to the variable
                    minimum_value=convert_value
                i=i+1

            #we then print the final minimum value
            print 'The minimum value is :%d' %minimum_value
            return 0
        #here is where we catch an invalid value or a keyboard interrupt
        except ValueError,KeyboardInterrupt:
            print 'Invalid value! Minimal was not possible.'
            parsed = True